    ...
    >
      {numSelected > 0 ? (
        ...
        >
          ...
          {`ALL EVENTS ${props.currentBoat ? (props.currentBoat) : (props.curCamera.description)}`}
        </Typography>
      )}
...
export const BoatEvents = (props) => {
  ...
  useEffect((e) => {
    ...
    if (props.currentBoat) setData(props.events.filter(e => e.typeVessel === props.currentBoat));
    else setData(props.events);

    if (props.selectedImage >= 0) {
      setSelected([props.selectedImage]);
    }
  }, [props.currentBoat, props.selectedImage, props.events]);
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        <EnhancedTableToolbar
          ...
          currentBoat={props.currentBoat}
          curCamera={props.curCamera}
        />
        <TableContainer>
          <Table
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  const notifType = `events__type__notification ${row.typeError.toLowerCase()}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.id, (index + page*rowsPerPage))}
                      role="checkbox"
